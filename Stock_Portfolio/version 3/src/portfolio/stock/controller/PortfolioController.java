...
public class PortfolioController implements IPortfolioController {
  ...
  protected void buyStocks() {
    ...
    double value;
    try {
      value = Double.parseDouble(view.getBuyPrice());
      if (value == -999) {
        return;
      }
   ...
   */
  protected void getPortfolioContents() {
    String name = view.getPortfolioName();
    if (name.equals("")) {
      return;
    }
  ...
  private void inputType() {
    ...
    try {
      while (firstInput) {
        ...
        System.out.println(optionAcceptInput);
